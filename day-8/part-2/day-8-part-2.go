	...
	for _, satellite := range(sat_array) {
		...
		for index, location := range(satellite) {
			...
			for other_index, other_location := range(satellite) {
				...
				fmt.Println(location, other_location)
				...
				loc_x := location[0] - dist_x
				loc_y := location[1] - dist_y

				for true {

					fmt.Println(loc_x, loc_y)
					if loc_y >= len(grid) || loc_x >= len(grid[0]) ||
					loc_x < 0 || loc_y < 0 {
	
						break
					}
					if node_grid[loc_y][loc_x] == false {
						
						node_grid[loc_y][loc_x] = true
						node_count += 1
					}
				...
				if node_grid[location[1]][location[0]] == false {

					node_count += 1
					node_grid[location[1]][location[0]] = true
				}
			}
		}
	}

	for _, row := range(node_grid) {
